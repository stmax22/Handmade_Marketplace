/* Скрипт обновления/добавления данных в таблицах измерений и фактов. */
BEGIN;

/* 1. Создание временной таблицы tmp_sources с данными из всех источников. */
...
JOIN source2.craft_market_orders_customers AS t2 USING(product_id, craftsman_id)
UNION
SELECT
	t1.order_id,
...
JOIN source3.craft_market_craftsmans AS t2 USING(craftsman_id)
JOIN source3.craft_market_customers AS t3 USING(customer_id)
UNION
SELECT
	t1.order_id,
    t1.order_created_date,
    ...
    t2.customer_birthday,
    t2.customer_email 
FROM external_source.craft_products_orders AS t1 
JOIN external_source.customers AS t2 USING(customer_id);

/* 2. Обновление/добавление данных в таблице измерения dwh.d_craftsmans. */
...
/* 3. Обновление/добавление данных в таблице измерения dwh.d_products. */
...
/* 4. Обновление/добавление данных в таблице измерения dwh.d_customer. */
...
/* 5. Создание временной таблицы tmp_sources_fact. */
...
JOIN dwh.d_craftsman AS dc ON dc.craftsman_name = src.craftsman_name
	AND dc.craftsman_email = src.craftsman_email 
JOIN dwh.d_customer AS dcust ON dcust.customer_name = src.customer_name
	AND dcust.customer_email = src.customer_email 
JOIN dwh.d_product AS dp ON dp.product_name = src.product_name
	AND dp.product_description = src.product_description
	AND dp.product_price = src.product_price;

/* 6. Обновление/добавление данных в таблице фактов dwh.f_order. */
